...
if (!health.payload.services.openai.configured) {
  ...
  assert(generated.status === 503, "Report generation should fail without OpenAI key even when local mode is requested.");
  assert(generated.payload.message.includes("OPENAI_API_KEY"), "Missing key error should mention OPENAI_API_KEY.");
}
