...
function productArchetype(context) {
  ...
  if (/剃|刮毛|脱毛|美容|美妆|个护|groom|beauty|razor|shaver|hair removal/.test(text)) {
    return {
      ...
      visualHook: "使用前后对比、细节质感、便携收纳和真实上脸/上手效果",
...
function directFallbackConclusion(module, label, context, index = 0) {
  ...
  const profile = productArchetype(context);

  if (type === "market_intelligence") {
    if (textIncludesAny(label, ["tam", "总可用"])) return `TAM结论：${context.market}${profile.categoryRole}的大盘需求来自${profile.demandDrivers}，用于判断市场上限，不直接等同首年销量。`;
    if (textIncludesAny(label, ["sam", "可服务"])) return `SAM结论：可服务市场应聚焦${context.platforms}中愿意接受${context.price}价格带的${context.consumers}。`;
    if (textIncludesAny(label, ["som", "可获得"])) return `SOM结论：首阶段可获得市场来自达人内容转化、搜索承接和竞品差评切入，建议先按小批量份额评估。`;
    if (textIncludesAny(label, ["amazon"])) return `Amazon销量结论：Amazon更适合承接搜索型需求，重点观察“${profile.searchTerms}”相关关键词和同价位Listing转化。`;
    if (textIncludesAny(label, ["tiktok"])) return `TikTok销量结论：TikTok增量来自${profile.visualHook}，短期爆发取决于视频完播率、互动率和达人转化。`;
    if (textIncludesAny(label, ["walmart"])) return `Walmart销量结论：Walmart适合做价格比较和家庭消费承接，但爆发性通常弱于TikTok，适合作为补充渠道。`;
    if (textIncludesAny(label, ["google", "trends", "趋势"])) return `趋势结论：搜索趋势应重点观察${profile.seasonality}，并对比${context.category}关键词的近5年热度。`;
    if (textIncludesAny(label, ["季节"])) return `季节性结论：${context.product}的排期应围绕${profile.seasonality}提前4-6周准备内容、库存和达人。`;
    if (textIncludesAny(label, ["价格带", "价格"])) return `价格带结论：${context.price}价格需要由${profile.visualHook}和售后承诺支撑，否则容易被低价竞品压制。`;
    if (textIncludesAny(label, ["品牌集中"])) return `品牌集中度结论：若头部品牌评价多但内容表达弱，TikTok仍可通过${profile.visualHook}切入。`;
    if (textIncludesAny(label, ["top100", "竞品"])) return `TOP竞品结论：优先拆解同价位竞品的${profile.painPoints}，用改进点做Listing和短视频卖点。`;
    if (textIncludesAny(label, ["店铺"])) return `店铺分布结论：若竞品集中在Amazon，TikTok Shop可用达人内容降低新品冷启动成本。`;
    if (textIncludesAny(label, ["利润率", "利润"])) return `利润率结论：出厂成本${context.cost}、售价${context.price}下裸毛利约${margin}，核心压力来自履约、广告、佣金和退货。`;
    ...
    return fallbackVariant([
      ...
      `${context.product}测品成败取决于${profile.visualHook}能否覆盖${context.consumers}并压低退货率。`
    ], index);
  }

  if (type === "creator_intelligence") {
    if (textIncludesAny(label, ["达人类型", "creator type"])) return `优先合作${profile.creatorTypes}；用${profile.visualHook}做内容钩子，中腰部达人负责转化。`;
    if (textIncludesAny(label, ["年龄"])) return `核心粉丝年龄应围绕${context.consumers}判断，优先验证25-44岁主力购买人群和礼赠/自用人群差异。`;
    if (textIncludesAny(label, ["性别"])) return `粉丝性别应由${context.category}实际购买场景决定，内容要同时测试自用、送礼和家庭决策三类表达。`;
    if (textIncludesAny(label, ["地区"])) return `地区优先级应结合${context.market}消费能力、物流覆盖和${profile.seasonality}来排序。`;
    if (textIncludesAny(label, ["消费能力"])) return `${context.price}售价属于中高客单，建议匹配家庭收入$75k+、愿意为${context.category}的${profile.demandDrivers}付费的人群。`;
    if (textIncludesAny(label, ["兴趣", "标签"])) return `核心兴趣标签应围绕${context.category}、${context.scenarios}、${profile.searchTerms}和目标人群生活方式设置。`;
    if (textIncludesAny(label, ["爆款率"])) return `内容爆款率取决于${profile.visualHook}是否能在前3秒讲清楚差异化和购买理由。`;
    if (textIncludesAny(label, ["佣金"])) return `平均佣金建议设为15%-20%；${context.price}客单价可支撑达人测评成本，但需控制样品和物流费用。`;
    if (textIncludesAny(label, ["合作难度"])) return `合作难度预计中等：${profile.creatorTypes}通常需要明确佣金、样品价值、卖点素材和使用脚本。`;
    if (textIncludesAny(label, ["百万", "GMV"])) return `按${context.price}售价测算，100万美元GMV约需售出${units}台；建议准备120-200位达人池分层测试。`;
    return fallbackVariant([
      `达人策略以${profile.creatorTypes}为主，先测内容转化，再放大高ROI达人。`,
      "达人分层建议先用长尾达人验证脚本，再用腰部达人放大销量，头部达人只在转化稳定后合作。",
      `达人内容必须展示${profile.visualHook}，单纯口播不适合作为主素材。`
    ], index);
  }
  ...
  if (type === "consumer_intelligence") {
    ...
    if (textIncludesAny(label, ["购买", "原因"])) return `购买理由集中在${profile.demandDrivers}，需要用内容验证自用、送礼和场景触发哪个转化最高。`;
    if (textIncludesAny(label, ["痛点"])) return `核心痛点是${profile.painPoints}，详情页和评论区必须提前解释。`;
    return fallbackVariant([
      `${context.product}目标用户应锁定${context.consumers}，不要套用其他品类画像。`,
      `用户购买前会重点比较${profile.painPoints}和${context.price}价格合理性。`,
      `用户转化内容应分别覆盖${context.scenarios}中的高频使用场景。`
    ], index);
  }

  if (type === "video_ai") {
    if (textIncludesAny(label, ["选题"])) return `爆款选题围绕${profile.visualHook}、${profile.demandDrivers}和“为什么现在需要它”展开。`;
    if (textIncludesAny(label, ["脚本", "30", "45", "60"])) return `脚本结构：3秒展示${profile.visualHook}，中段解释${profile.painPoints}解决方案，结尾给出购买理由和CTA。`;
    if (textIncludesAny(label, ["分镜", "镜头"])) return `分镜优先拍${profile.visualHook}、真实使用场景、细节特写和用户反馈，避免只拍产品静物。`;
    if (textIncludesAny(label, ["bgm", "字幕"])) return `字幕突出${context.category}核心卖点、${context.price}价格理由和${context.consumers}最关心的问题。`;
    return fallbackVariant([
      `${context.product}短视频核心是${profile.visualHook}，第一屏必须让用户知道差异化价值。`,
      `短视频应把${context.scenarios}拆成多个可拍脚本，避免所有视频只展示产品外观。`,
  ...
  if (type === "live_ai") {
    ...
    if (textIncludesAny(label, ["演示"])) return `直播必须现场演示${profile.visualHook}，证明产品解决${profile.painPoints}。`;
    if (textIncludesAny(label, ["问题", "回答"])) return `高频问答聚焦${profile.painPoints}、适用人群、售后政策和与竞品差异。`;
    return fallbackVariant([
  ...
  if (type === "comment_ai") {
    if (textIncludesAny(label, ["喜欢", "好评"])) return `好评卖点应围绕${profile.visualHook}、使用体验和${profile.demandDrivers}。`;
    if (textIncludesAny(label, ["退货", "差评"])) return `差评风险集中在${profile.painPoints}和售后响应。`;
    if (textIncludesAny(label, ["文案"])) return `营销文案应强调${context.product}如何解决${profile.painPoints}，并给出清晰使用场景。`;
    return fallbackVariant([
      `${context.product}评论分析要把正向卖点转成视频脚本，把差评风险转成详情页FAQ和售后承诺。`,
      `评论抓取应优先看${profile.painPoints}，这些会直接影响退货。`,
  ...
  if (type === "compliance_ai") {
    ...
    if (textIncludesAny(label, ["违规", "风险"])) return `合规风险主要来自${profile.complianceFocus}。`;
    if (textIncludesAny(label, ["知识产权", "专利", "商标"])) return `需核查外观结构、功能设计、品牌词和竞品图片版权，避免直接复制爆款素材。`;
    return fallbackVariant([
      `${context.product}的合规重点是${profile.complianceFocus}。`,
  ...
  if (type === "decision_center") {
    ...
    return fallbackVariant([
      ...
      `如果退货率超过预期，应暂停补货并优先处理${profile.painPoints}。`
    ], index);
  }
